"""Zero-One-Random GT | supervised | unsupervised comparison.

The zero_one_random analogue of fig1_mess3_principled.py (Figure 1 in writeup_v1.tex):
a 3-panel belief-geometry comparison.
...
zero_one_random has 3 hidden states, so its belief lives on the same 2-simplex triangle as
Mess3 and reuses the identical projection / datashader raster / belief-decode metric. Same
enumerated MSP prefixes, same simplex projection, same label placement (display only).

Uses the zero_one_random checkpoint (the overnight run was interrupted by a Windows Update
reboot at ~200k steps; the model is already at its optimal windowed loss, so the belief
...
def load_model(device):
    ckpt = torch.load(MODEL_DIR / "zero_one_random_transformer.pt", map_location=device, weights_only=False)
...
def main():
    ...
    hmm = build_hidden_markov_model("zero_one_random", ZERO_ONE_RANDOM_PARAMS)
    vocab = hmm.vocab_size
    model, ctx, step = load_model(device)
    print(f"loaded zero_one_random checkpoint (step={step}, ctx={ctx}, device={device})")

    resid, soft, belief, prefix_prob, max_len = U.collect_prefix_features_enumerated(model, hmm, ctx, device)
    # zero_one_random has a deterministic backbone, so many token prefixes are impossible
    # (prob 0 / NaN belief in the MSP tree). Keep only reachable prefixes for the geometry.
    good = {
        w for w in resid
    ...
    print(f"zero_one_random belief-decode R^2:  supervised(64-D)={r2_sup:.3f}   unsupervised({D}-D)={r2_uns:.3f}")

    fig, ax = plt.subplots(1, 3, figsize=(18, 6))
    ax[0].imshow(U.rasterize_simplex(true_xy, color, px=2), origin="lower")
    ax[0].set_title("Zero-One-Random ground truth")
    ...
    for a in ax:
        a.axis("off")
    out = FIG_DIR / "fig1_zero_one_random_principled.png"
