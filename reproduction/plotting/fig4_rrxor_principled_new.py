"""fig4-style RRXOR belief-geometry figure for the NEW model (Adam, ctx8, 4M updates).

Identical construction to fig4_rrxor_principled.py -- same per-(input, position) data, same
per-state-equal-weighted affine readout, same PC1/PC3 display -- but on the newly trained
context-8 model in `models/`, and using the PLAIN spectral-OOM (depth-3 observability matrix,
P(w)-weighted ridge, NO multistep-ALS), i.e. the validated baseline that reproduces the
write-up's 0.65 on the old ctx10 model.

Panel 2 (supervised): full concatenated residual stream -> belief.
Panel 3 (unsupervised): recovered obs-OOM 5-D subspace -> belief.
"""
...
MODEL_DIR = Path(__file__).parent.parent / "models"          # NEW Adam/ctx8 models
FIG_DIR = Path(__file__).parent.parent / "figures"
CTX = 8                                                       # new model context window
D = 5
...
def main():
    ...
    # ---- unsupervised subspace B: PLAIN spectral-OOM (depth 3, P(w)-weighted, no ALS) ----
    ...
    _, _, _, _, Uobs, _ = F14.observable_subspace(resid, soft, reach, 2, depth=3, wmap=pw,
                                                  use_multistep_als=False)
    B = Uobs[:, :D]
    ...
    print(f"per-position decode R^2 (per-state-equal wt): supervised={r2_sup:.3f}  unsupervised(5-D)={r2_uns:.3f}")
    ...
    fig.suptitle("RRXOR belief geometry -- NEW model (Adam, context 8, 4M updates)",
                 fontsize=14, y=1.02)
    ...
    out = FIG_DIR / "fig4_rrxor_principled_new_adam_ctx8.png"
