"""fig4-style RRXOR belief-geometry figure for the NEW model (Adam, ctx14, 4M updates).

Identical construction to fig4_rrxor_principled_new.py -- same per-(input, position) data, same
per-state-equal-weighted affine readout, same PC1/PC3 display, and the SAME validated PLAIN
spectral-OOM baseline (depth-3 observability matrix, P(w)-weighted ridge, NO multistep-ALS)
that reproduces the write-up's 0.65 on the old ctx10 model -- but pointed at the newly trained
context-14 model in `models/`.
...
def main():
    ...
    fig.suptitle("RRXOR belief geometry -- NEW model (Adam, context 14, 4M updates)",
                 fontsize=14, y=1.02)
    ...
    out = FIG_DIR / "fig4_rrxor_principled_adam_ctx14.png"
