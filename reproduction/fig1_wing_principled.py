"""Wing GT | supervised | unsupervised comparison.

The wing analogue of fig1_mess3_principled.py (Figure 1 in writeup_v1.tex): a 3-panel
...
wing has 3 hidden states (vocab 2), so its belief lives on the same 2-simplex triangle as
Mess3 and reuses the identical projection / datashader raster / belief-decode metric. wing is
non-unifilar, so its belief should not collapse to the vertices.

Uses the wing checkpoint (the overnight run was interrupted by a Windows Update reboot at
...
def main():
    ...
    print(f"wing belief-decode R^2:  supervised(64-D)={r2_sup:.3f}   unsupervised({D}-D)={r2_uns:.3f}")
    ...
    for a in ax:
        a.axis("off")
    out = FIG_DIR / "fig1_wing_principled.png"
