"""Train a transformer on arch — EXACT continuous-stream pipeline (paper / epsilon-transformers).
...
decay, 1,000,000 steps. Arch a=0.85 (vocab 3, states 4).
...
OPTIMAL_LOSS = 1.0143          # optimal windowed CE (myopic entropy avg over ctx 1..10, belief reset per window); old 1.0451 was the unigram/no-context entropy (asymptotic entropy rate = 1.0107)

OUT_DIR = Path(__file__).parent
MODEL_DIR = OUT_DIR / "models"
FIG_DIR = OUT_DIR / "figures"
CKPT = MODEL_DIR / "arch_transformer.pt"
...
def save_ckpt(model, losses, step):
    torch.save(
        {"state_dict": model.state_dict(), "cfg": model.cfg.to_dict(), "losses": losses,
         "arch_params": ARCH_PARAMS, "context_len": CONTEXT_LEN, "step": step},
        CKPT,
    )
...
def main() -> None:
    ...
    ax.set_title(f"arch (paper config, continuous stream) training (final {losses[-1]:.4f})")
    ax.legend()
    fig.savefig(FIG_DIR / "fig_arch_training_loss.png", dpi=150, bbox_inches="tight")
