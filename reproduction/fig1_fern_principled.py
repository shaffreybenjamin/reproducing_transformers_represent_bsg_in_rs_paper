"""Fern GT | supervised | unsupervised comparison.

The fern analogue of fig1_mess3_principled.py / fig1_zero_one_random_principled.py
(Figure 1 in writeup_v1.tex): a 3-panel belief-geometry comparison.
...
fern has 3 hidden states (vocab 2), so its belief lives on the same 2-simplex triangle as
Mess3 and reuses the identical projection / datashader raster / belief-decode metric. fern is
non-unifilar, so (unlike zero_one_random) its belief should not collapse to the vertices.

Uses the fern checkpoint (the overnight run was interrupted by a Windows Update reboot at
...
def main():
    ...
    print(f"loaded fern checkpoint (step={step}, ctx={ctx}, device={device})")
    ...
    print(f"fern belief-decode R^2:  supervised(64-D)={r2_sup:.3f}   unsupervised({D}-D)={r2_uns:.3f}")

    fig, ax = plt.subplots(1, 3, figsize=(18, 6))
    ax[0].imshow(U.rasterize_simplex(true_xy, color, px=2), origin="lower")
    ax[0].set_title("Fern ground truth")
    ...
    for a in ax:
        a.axis("off")
    out = FIG_DIR / "fig1_fern_principled.png"
