"""Train a transformer on strata — EXACT continuous-stream pipeline (paper / epsilon-transformers).
...
decay, 1,000,000 steps. Strata a=0.85, t0=0.5, t1=0.5 (vocab 2, states 3).
...
CKPT = MODEL_DIR / "strata_transformer.pt"
...
def save_ckpt(model, losses, step):
    torch.save(
        {"state_dict": model.state_dict(), "cfg": model.cfg.to_dict(), "losses": losses,
         "strata_params": STRATA_PARAMS, "context_len": CONTEXT_LEN, "step": step},
        CKPT,
    )
...
def main() -> None:
    ...
    ax.set_title(f"strata (paper config, continuous stream) training (final {losses[-1]:.4f})")
    ax.legend()
    fig.savefig(FIG_DIR / "fig_strata_training_loss.png", dpi=150, bbox_inches="tight")
