"""Train a transformer on wing — EXACT continuous-stream pipeline (paper / epsilon-transformers).
...
decay, 1,000,000 steps. Wing x=0.5, y=0.5 (vocab 2, states 3).
...
OPTIMAL_LOSS = 0.6026          # optimal windowed CE (myopic entropy avg over ctx 1..10, belief reset per window); old 0.6036 was the unigram/no-context entropy (asymptotic entropy rate = 0.6024)
...
def main() -> None:
    ...
    ax.set_title(f"wing (paper config, continuous stream) training (final {losses[-1]:.4f})")
    ax.legend()
