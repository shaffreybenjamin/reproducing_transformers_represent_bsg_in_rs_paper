"""Train a transformer on RRXOR with the QUANTUM-PAPER training setup.
...
from the enumerated length-(n_ctx+1) window distribution. RRXOR has forbidden
transitions, so the enumeration keeps only reachable windows (prob > 0); the
optimal-loss line is the exact myopic-entropy bound for this context length.

RRXOR: p1=0.5, p2=0.5 (vocab 2, 5 hidden states).
"""
...
def main() -> None:
    ...
    data = build_process_data(PROCESS, RRXOR_PARAMS, CONTEXT_LEN, device)
    ...
    ax.set_title(f"RRXOR (quantum-paper config) training (final {epoch_losses[-1]:.4f})")
    ax.legend()
