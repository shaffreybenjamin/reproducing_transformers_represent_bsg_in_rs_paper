"""Train a transformer on zero_one_random — EXACT continuous-stream pipeline (paper / epsilon-transformers).
...
decay, 1,000,000 steps. Zero-One-Random p=0.5 (vocab 2, states 3).
...
OPTIMAL_LOSS = 0.2947          # optimal windowed CE (myopic entropy avg over ctx 1..10, belief reset per window); old 0.6931 was the unigram/no-context entropy (asymptotic entropy rate = ln(2)/3 = 0.2311)

OUT_DIR = Path(__file__).parent
MODEL_DIR = OUT_DIR / "models"
FIG_DIR = OUT_DIR / "figures"
CKPT = MODEL_DIR / "zero_one_random_transformer.pt"
...
def save_ckpt(model, losses, step):
    torch.save(
        {"state_dict": model.state_dict(), "cfg": model.cfg.to_dict(), "losses": losses,
         "zero_one_random_params": ZERO_ONE_RANDOM_PARAMS, "context_len": CONTEXT_LEN, "step": step},
        CKPT,
    )
...
def main() -> None:
    ...
    ax.set_title(f"zero_one_random (paper config, continuous stream) training (final {losses[-1]:.4f})")
    ax.legend()
    fig.savefig(FIG_DIR / "fig_zero_one_random_training_loss.png", dpi=150, bbox_inches="tight")
